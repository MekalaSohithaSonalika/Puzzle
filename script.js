document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleTouchEnd(e) {
        if (longPressTimer) {
            ...
            // Handle as normal tap if not dragging
            if (!isDragging) {
                const piece = e.target.closest('.puzzle-piece');
                handlePieceClick({ target: piece });
            }
        }
    ...
    function startDragFromTouch(e, piece) {
        if (!piece) return;
        ...
        if (wasInGrid) {
            convertToAbsolutePosition(draggedPiece, touch.clientX, touch.clientY);
        }
